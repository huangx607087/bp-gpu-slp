...
const u64 ZERO4CPY=0;
u64 *Wgpu[9]={nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr};
u64 *W[9]={nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr,nullptr};
unsigned long long Wlen[9]={0,0,0,0,0,0,0,0,0};
void combEntry(int baseNumber,int combNumber)
{
	...
	Wlen[combNumber]=C[baseNumber*9+combNumber];
	
	if(Wgpu==nullptr) cudaMalloc(&Wgpu[combNumber],4ULL*C[baseNumber*9+combNumber]);
	getXorSumKernel<<<gridSize,blockSize>>>(Wgpu[combNumber],Bgpu,CDatagpu,C[baseNumber*9+combNumber],combNumber);
	cudaFree(CDatagpu);
}
...
void newDistancePre()
{
	...
	{
		printf("%d",i);
		cudaMemcpy(W[i],Wgpu[i],4ULL*Wlen[i],cudaMemcpyDeviceToHost);//O(N)
		sort(W[i],W[i]+Wlen[i]);//O(N log N)
		cudaMemcpy(Wgpu[i],W[i],4ULL*Wlen[i],cudaMemcpyHostToDevice); //O(N)
	}
}

__global__ void binFindKernel(u64 *W1,u64 *W2,u64 Tx,unsigned long long lenW1,unsigned long long lenW2,int *resaddr)
{
	unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
	if(idx>=lenW2) return;
	u64 Target=(Tx^W2[idx]);
	unsigned long long  L=0,R=lenW1-1;
	while(L<=R)
	{
...
bool reachableFaster(u64 T,int K)
{
	...
	// printf("%d %d %p %p\n",Wlen[K1],Wlen[K2],W[K1],W[K2]);
	int *resgpu,res=0;

	cudaMalloc(&resgpu,4);
	cudaMemcpy(resgpu,&ZERO4CPY,4,cudaMemcpyHostToDevice);

	int blockSize=256;
	unsigned long long gridSize = (Wlen[K2]+blockSize-1)/ blockSize;
	binFindKernel<<<gridSize,blockSize>>>(Wgpu[K1],Wgpu[K2],T,Wlen[K1],Wlen[K2],resgpu);
	
	cudaMemcpy(&res,resgpu,4,cudaMemcpyDeviceToHost);
	cudaFree(resgpu);
	// printf("%d ",res);
	if(res) return 1;
...
void dealBP10()
{
	...
	sprintf(OUTFILENAME,"Run%02d%016llx%04dresult.txt",TIMUID,RUNID,++REC);
	sprintf(LOGFILENAME,"Run%02d%016llx%04dlog.txt",TIMUID,RUNID,REC);
	FILE *fp,*flp;
	...
	for(;;)
	{
		...
		flp=fopen(LOGFILENAME,"a");
		fprintf(flp,"Last Time Cost: %08llu ms\n",duration.count());
		T1=T2;
		newDistancePre();
		printf("culcated dis.");
		...
		for(int i=0;i<n;i++)
		{
			...
			fprintf(flp,"%d%c",D[i],(i+1==n)?10:32);
		}
		fclose(flp);
		if(!sumDist) break;
		int found=0;
...
int main()
{
	...
	cudaMalloc(&Cgpu,MAXM*72);
    cudaMemcpy(Cgpu,C,MAXM*72,cudaMemcpyHostToDevice);
